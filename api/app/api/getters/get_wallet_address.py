...
async def get_wallet_address(dao: str, owner: str) -> WalletAddressResponse:
    ...
    async with limiter:
        result = await tonapi_client.blockchain.execute_get_method(
            addr.to_string(), "get_wallet_address", cell.to_boc().hex()
        )
    if not result.success:
        raise Exception(result)
    addr = (
    ...
    return WalletAddressResponse(address=APIAddress.from_address(addr))
