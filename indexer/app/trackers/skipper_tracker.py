...
from indexer.app.infra.ton.parsers.skipper import (
    parse_skipper_trace,
    NewProposalState,
    VoteProposalState,
)
...
class SkipperTracker(BaseTracker):
            ...
            while True:
                try:
                    ...
                    parsed_trace = await parse_skipper_trace(
                        dao.address, self.ctx.tonapi_client, trace_info
                    )
                    ...
                    if isinstance(parsed_trace, NewProposalState) or isinstance(
                        parsed_trace, VoteProposalState
                    ):
    ...
    async def _track_dao(self, dao: DAO):
        ...
        while True:
            ...
            logger.debug(
                "Last trace for dao %s is %s",
                dao.address.to_string(),
                last_trace.hash if last_trace else None,
            )
            try:
                traces = await list_new_traces(
                    self.ctx.tonapi_client, last_trace, dao.address
                )
