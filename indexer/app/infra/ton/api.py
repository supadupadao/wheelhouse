    ...
    while True:
        async with limiter:
            result = await tonapi_client.accounts.get_traces(
                address.to_string(), limit=MAX_PER_PAGE, before_lt=before_lt
            )
...
async def list_jetton_holders(
    tonapi_client: AsyncTonapi, jetton_master: Address
) -> list[JettonHolder]:
    result = []
    offset = 0
    while True:
        async with limiter:
            holders = await tonapi_client.jettons.get_holders(
                jetton_master.to_string(), MAX_PER_PAGE, offset
            )
